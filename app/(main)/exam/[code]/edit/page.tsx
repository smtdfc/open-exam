...
export default function EditExamPage({
  ...
  useEffect(() => {
    const loadExam = async () => {
      try {
        ...
        const response = await fetch(`/api/exams/${normalizedCode}?mode=edit`);
        ...
        if (!response.ok) {
          setErrorMessage(
            "message" in data
              ? data.message || "Không thể tải bài thi."
              : "Không thể tải bài thi.",
          );
          return;
        }

        const exam = (data as { exam: LoadedExam }).exam;
        if (!exam.viewer.isOwner) {
          setErrorMessage("Bạn không có quyền sửa bài thi này.");
          return;
        }

        setCanEditQuestions(exam.viewer.canEditQuestions);

        setExamSettings({
          title: exam.title,
          description: exam.description || "",
          duration: exam.durationMinutes || 60,
          unlimitedTime: exam.durationMinutes === 0,
          maxAttempts: exam.maxAttempts || 1,
          unlimitedAttempts: exam.maxAttempts === 0,
          allowResultReview: exam.allowResultReview,
          deadline: toDateTimeLocalValue(exam.expiresAt),
          isMonitored: exam.isMonitored,
          recordBehavior: exam.recordBehavior,
        });
  ...
  const handleSaveExam = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrorMessage(data.message || "Không thể lưu bài kiểm tra.");
        return;
      }

      setActionMessage(data.message || "Đã lưu thay đổi.");
    } catch {
      setErrorMessage("Không thể kết nối đến máy chủ.");
    } finally {
      setIsSaving(false);
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      <div className="flex justify-between items-center sticky top-0 bg-slate-50/80 backdrop-blur-md z-10 py-4 px-3 border-b border-slate-200">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Mã bài thi: <span className="font-semibold">{code}</span> · Tổng
            điểm: {questions.reduce((acc, curr) => acc + (curr.points || 0), 0)}
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <Button onClick={handleSaveExam} isLoading={isSaving}>
            Lưu thay đổi
          </Button>
        </div>
      </div>
      ...
      {!canEditQuestions && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Bài thi này đã có lượt làm. Bạn chỉ nên sửa các thông tin chung; thay
          đổi nội dung câu hỏi sẽ bị từ chối để bảo toàn dữ liệu đã nộp.
        </div>
      )}
